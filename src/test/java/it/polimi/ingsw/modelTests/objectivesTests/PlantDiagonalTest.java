 ...
 */
public class PlantDiagonalTest
{
    /**
     * Test the PatternFinder of the diagonal pattern of
     * the plant kingdom:<ul>
     ...
     * calculate the gained point for the diagonal pattern (three plant
     ...
     */
    @Test
    @DisplayName("Plant Diagonal Pattern on Diagonal Board")
    public void testOnDiagonal()
    {
        // Get the objective instance to test (plant diagonal,
        // objective #2).
        Objective target = Objective.getObjectiveWithId(1);

        // Get a plant card.
        KingdomCardFace animalCard = ResourceCard.getCardWithId(10).getFrontFace();
        ...
        // Place a plant card on the board.
        ...
        // Place a plant card on the board, creating a diagonal with
        // two cards (left hand diagonal).
        board.addCard(new BoardSlot(1,3), animalCard);
        ...
        // Place a plant card on the board, creating a diagonal with
        // three cards.
        board.addCard(new BoardSlot(0,4), animalCard);
        ...
        // Place a plant card on the board, creating a diagonal with
        // four cards.
        board.addCard(new BoardSlot(3,1), animalCard);

        // Test the pattern finder: expected 2x1=2 points.
        gainedPoints = target.calculatePoints(board);
        assertEquals(2, gainedPoints);

        // Place two plant cards on the board, creating a diagonal with
        // six cards.
        board.addCard(new BoardSlot(-1,5), animalCard);
        board.addCard(new BoardSlot(-2,6), animalCard);
        ...
        // Place a plant card on the board, creating a diagonal with
        // seven cards.
        board.addCard(new BoardSlot(-3,7), animalCard);
     ...
     * pattern (three plant cards aligned from bottom-left to top-right):<ul>
     *     <li>1x1 rectangle</li>
     *     <li>2x2 rectangle</li>
     *     <li>3x3 rectangle</li>
     *     <li>3x6 rectangle</li>
     ...
     */
    @Test
    @DisplayName("Plant Diagonal Pattern on Rectangular Board")
    public void testOnRectangularBoard() {
        ...
        DummyPlayingBoard board = DummyPlayingBoard.getUniformKingdomBoard(
                origin, 1, 1, Kingdom.PLANT_KINGDOM
        );
        ...
        board = DummyPlayingBoard.getUniformKingdomBoard(
                origin, 2, 2, Kingdom.PLANT_KINGDOM
        );
        ...
        board = DummyPlayingBoard.getUniformKingdomBoard(
                origin, 3, 3, Kingdom.PLANT_KINGDOM
        );
         ...
         */
        board = DummyPlayingBoard.getUniformKingdomBoard(
                origin, 3, 6, Kingdom.PLANT_KINGDOM
        );
         ...
         */
        board = DummyPlayingBoard.getUniformKingdomBoard(
                origin, 6, 6, Kingdom.PLANT_KINGDOM
        );
