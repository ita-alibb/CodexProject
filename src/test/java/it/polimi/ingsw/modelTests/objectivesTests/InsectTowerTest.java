...
/**
 * Unit test for Objective class, insect tower game objective.
 */
public class InsectTowerTest {
    ...
    /**
     * The objective to test: Tower Pattern of Insect Kingdom (objective Id = 7).
     */
    private static final Objective TARGET = Objective.getObjectiveWithId(7);
    /**
     * The card used in the tower: card with Insect kingdom.
     */
    private static final KingdomCardFace TOWER_CARD = ResourceCard.getCardWithId(30).getFrontFace();
    /**
     * The card used in the base: card with Animal kingdom.
     */
    private static final KingdomCardFace BASE_CARD = ResourceCard.getCardWithId(20).getFrontFace();

    //endregion

    /**
     * The insect tower pattern is composed of two cards of the insect
     * kingdom aligned in vertical direction, with an animal kingdom
     * card placed in the top left corner of the tower.
    ...
    public void testTowerLength() {
        ...
        // 2) Place an insect card in the reference slot.
        ...
        // 3) Place an insect card at the bottom of the first card (create the tower
        // without the base).
        ...
        // 4) Place an animal card in the top-left corner, to create
        // a complete tower pattern (tower + base).
        board.addCard(refSlot.getSlotAt(TOP_LEFT), BASE_CARD);

        // Test the pattern finder: expected 3 points.
        gainedPoints = TARGET.calculatePoints(board);
        assertEquals(3, gainedPoints);

        // 5) Place an additional insect card at the top of the tower.
        ...
        // 6) Place an additional insect card at the bottom of the tower.
    ...
    public void testTowerBase() {
        ...
        // Create the tower placing two insect card aligned in vertical
        // direction.
        board.addCard(refSlot, TOWER_CARD);
        board.addCard(refSlot.getSlotAt(BOTTOM), TOWER_CARD);
        ...
        // 1.d) Remove wrong base and add a new base at the (correct) top-left
        // corner.
        board.removeCard(wrongBase);
    ...
    public void testDoublePattern(){
        ...
        DummyPlayingBoard board = DummyPlayingBoard.getNewBoardWithTowerPattern(
                refSlot, TOWER_CARD, BASE_CARD, CornerLocation.TOP_LEFT
        );
        ...
        // Place the pattern.
        DummyPlayingBoard.placeTowerPattern(board, topOrigin, TOWER_CARD, BASE_CARD, CornerLocation.TOP_LEFT);
        ...
        // Place the pattern.
        DummyPlayingBoard.placeTowerPattern(board, rightOrigin, TOWER_CARD, BASE_CARD, CornerLocation.TOP_LEFT);
    ...
    /**
     * Test the insect tower pattern on a striped bord, with alternating kingdom
     * along the columns.
     */
    @Test
    ...
    public void testStripedBoard() {
        ...
        // Check number of patterns: expected 6 patterns, 8 x 3 = 24 points.
        assertEquals(24, TARGET.calculatePoints(board));
    }

    /**
     * Test the insect tower pattern with the pattern having the wrong kingdom
     ...
     */
    @Test
    @DisplayName("Insect Tower Pattern: Wrong Base Kingdom")
