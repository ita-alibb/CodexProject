...
public class TowerPattern extends PatternFinder {
    ...
    /**
     * The height of the tower, i.e. the number of cards in vertical
     * direction.
     */
    ...
    /**
     * The kingdom of the cards composing the tower of the pattern, i.e. the
     * card aligned in vertical direction.
     */
    ...
    /**
     * This method checks if there is any corresponding base at the correct location,
     * in order to create one or more tower patterns, given a list of cards aligned in
     * vertical direction.
    ...
    private static int findMatchingBase(BoardInfo board, List<BoardSlot> tower, CornerLocation baseLocation,
        ...
        if (tower.size() < TOWER_HEIGHT) {
            // Return the counter, that is initialized to zero.
            return patterns;
        }

        // Check if there is a matching base to form a tower pattern.
        // Start at the first card of the tower.
        int baseIndex = 0;
        while (baseIndex < tower.size() - (TOWER_HEIGHT - 1)) {

            // The slot that is the candidate to be the base of
            // the tower pattern.
            BoardSlot base = tower.get(baseIndex);

            // Get the (optional) card placed on the base slot location.
            Optional<KingdomCardFace> baseCard = board.getNeighborCard(base, baseLocation);
            ...
            if (baseCard.isPresent() && baseCard.get().getKingdom().equals(baseKingdom)) {
                ...
                // Increment the index of the base slot, depending on the tower
                // height, to skip search for the all cards composing this pattern.
                baseIndex += TOWER_HEIGHT - 1;
            }

            // Increment the index of the base slot.
            baseIndex++;
        }
